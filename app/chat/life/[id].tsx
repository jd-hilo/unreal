...
export default function LifeChatScreen() {
  ...
  async function loadData() {
    ...
    setLoading(true);
    try {
      const [chatData, pack] = await Promise.all([
        getLifeChat(id, user.id),
      ...
      console.error('Failed to load life chat:', e);
    } finally {
      setLoading(false);
    }
  }
    ...
    if (!loading && initialMessage && typeof initialMessage === 'string' && !initialMessageSent.current && corePack) {
      ...
      (async () => {
        ...
        try {
          ...
          setIsStreaming(false);
          setStreamingMessage('');
          appendMessage('architect', "I'm having trouble responding right now. Please try again.");
        } finally {
          setSending(false);
        }
      })();
  ...
  async function handleSend() {
    ...
    try {
      await saveLifeChatMessage(id, user.id, 'user', trimmed);
    } catch (e) {
    ...
    try {
      ...
      try {
        await saveLifeChatMessage(id, user.id, 'architect', reply);
      } catch (e) {
        console.error('Failed to save architect message:', e);
      }
    } catch (e) {
      console.error('Send life chat error:', e);
      setIsStreaming(false);
      setStreamingMessage('');
      appendMessage('architect', "I'm having trouble responding right now. Please try again.");
    } finally {
